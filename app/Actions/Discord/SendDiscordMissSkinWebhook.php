...
final class SendDiscordMissSkinWebhook
{
     ...
     * @return void
     */
    public function __invoke(string $url, bool $isUnity)
    {
        $winners = SkinWinner::query()
            ->addSelect([
                'skin_image_path' => DB::table(($isUnity) ? 'unity_skins' : 'skins')
                    ->select('image_path')
                    ->whereColumn('id', 'skin_winners.skin_id')
                    ->take(1),
            ])
            ->when($isUnity, function ($query) {
                $query->skip(3);
            })
            ->take(3)
            ->get();
        ...
        $body = [
            'embeds' => [
                [
                    'title' => '1ère place '.(($isUnity) ? 'UNITY ' : '').'- Ocre',
                ...
                [
                    'title' => '2ème place '.(($isUnity) ? 'UNITY ' : '').'- Émeraude',
                ...
                [
                    'title' => '3ème place '.(($isUnity) ? 'UNITY ' : '').'- Cawotte',

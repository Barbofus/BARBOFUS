<div class="flex justify-center mt-36 mb-10">
    ...
    <div
        class="w-[80%] flex overflow-hidden" x-cloak
        x-data="{
            initButtonClass: 'border-b-2 text-inactiveText border-primary px-6 h-[65px] flex gap-x-2 items-center text-left text-xl w-full hover:bg-primary-100 hover:italic hover:text-secondary [&.active]:font-normal [&.active]:border-secondary [&.active]:text-secondary [&.active]:text-2xl fill-inactiveText [&.active]:fill-secondary',

        }">

        {{-- Pseudo navbar pour afficher tel ou tel onglet, 100% AlpineJS --}}
        <div class="w-[375px] font-light flex-col flex">

            <a  href="{{ route('dashboarduserdetails.index') }}" class="{{ Route::is('dashboarduserdetails.index') ? 'active' : ''}}"
                :class="initButtonClass">

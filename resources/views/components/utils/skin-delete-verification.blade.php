    ...
    <div class="w-96 rounded-md bg-primary-100 shadow-lg flex flex-col items-center justify-center gap-y-8 p-8"
        ...
        <div class="flex justify-between w-full">
            ...
            @if(isset($useController))
                <form action="{{ route('skins.delete', $skin->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
            ...
            @else
                <button x-on:click="$wire.deleteSkin(skinDeleteID), skinDeleteID = null" class="uppercase text-xl text-primary px-4 py-2 rounded-md goldGradient hover:rounded-3xl transition-all hover:tracking-widest">{{ __('barbofus.buttonDelete') }}</button>
            @endif
        </div>
    </div>
</div>

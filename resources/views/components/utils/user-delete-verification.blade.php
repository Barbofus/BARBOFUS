<div class="fixed top-0 right-0 bottom-0 left-0 z-[999] bg-black/40 flex justify-center items-center">
    <div class="w-96 rounded-md bg-primary-100 shadow-lg flex flex-col items-center justify-center gap-y-8 p-8"
        ...
        <div class="flex justify-between w-full">
            <button x-on:click="deleteID = null" class="uppercase text-xl text-goldText border-goldText border-2 hover:rounded-3xl transition-all hover:tracking-widest px-4 py-2 rounded-md">Annuler</button>
            <button x-on:click="$wire.deleteUser(deleteID), deleteID = null" class="uppercase text-xl text-primary px-4 py-2 rounded-md goldGradient hover:rounded-3xl transition-all hover:tracking-widest">Supprimer</button>
        </div>
    </div>
</div>

<div id="twitch-container" class="relative w-full h-full min-h-[125px]" x-data="{ open: true, }">
    ...
    <div id="twitch-screen"  class="absolute bottom-0 left-0 w-full aspect-video group"
        ...
        @if(isset($canClose) && $canClose)
            <button class="opacity-0 group-hover:opacity-100 absolute top-0 right-0 p-2 h-12 w-12 bg-primary z-40 rounded-bl-xl hover:bg-primary-100 transition-all group"
                @click="open = false">
        ...
        <div class="bg-primary absolute flex justify-between -left-[3px] z-0 h-[calc(100%+6px+20px)] w-[calc(100%+6px)] -top-[23px] rounded-md">
            ...
            <a href="https://www.twitch.tv/barbe___douce" target="_blank" class="hover:text-goldTextLit hover:-translate-y-1 font-thin text-goldText text-[0.8rem] transition-all">Rejoins-moi sur Twitch !</a>

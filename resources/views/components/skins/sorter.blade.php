...
<div class="relative max-[850px]:order-first"
     x-data="{
              ...
              SetFirstSelection()
              {
                ...
                if(orderID == 1 && orderDir == 'DESC') this.selection = '{{ __('barbofus.inputSortedLiked') }}';
                if(orderID == 1 && orderDir == 'ASC') this.selection = '{{ __('barbofus.inputSortedDisliked') }}';
                ...
                if(orderID == 4 && orderDir == 'DESC') this.selection = '{{ __('barbofus.inputSortedMissskin') }}';
    ...
    <div class="flex items-center justify-around gap-x-2"
        ...
        <div class="w-48 right-0 top-[2.25rem] overflow-hidden rounded-b-md absolute bg-secondary-100 text-[1rem] text-primary font-light transition-all duration-200 cursor-pointer"
            ...
            <button class="w-full h-full px-4 py-1 hover:bg-secondary"
               x-on:mousedown="selection = '{{ __('barbofus.inputSortedLiked') }}'; sortAsc = false; showSort = false; $wire.SortBy(1, 'DESC'), AddParamToUrl('sort', '1,DESC'), window.scrollTo(0,0)">{{ __('barbofus.inputSortedLiked') }}</button>
            <button class="w-full h-full px-4 py-1 hover:bg-secondary"
               x-on:mousedown="selection = '{{ __('barbofus.inputSortedDisliked') }}'; sortAsc = true; showSort = false; $wire.SortBy(1, 'ASC'), AddParamToUrl('sort', '1,ASC'), window.scrollTo(0,0)">{{ __('barbofus.inputSortedDisliked') }}</button>
            <button class="w-full h-full px-4 py-1 hover:bg-secondary"
            ...
            <button class="w-full h-full px-4 py-1 hover:bg-secondary"
               x-on:mousedown="selection = '{{ __('barbofus.labelSkinClass') }}'; sortAsc = true; showSort = false; $wire.SortBy(3, 'ASC'), AddParamToUrl('sort', '3,ASC'), window.scrollTo(0,0)">{{ __('barbofus.inputSortedClasses') }}</button>
            <button class="w-full h-full px-4 py-1 hover:bg-secondary"
                    x-on:mousedown="selection = '{{ __('barbofus.inputSortedMissskin') }}'; sortAsc = true; showSort = false; $wire.SortBy(4, 'DESC'), AddParamToUrl('sort', '4,DESC'), window.scrollTo(0,0)">{{ __('barbofus.inputSortedMissskin') }}</button>
        </div>
    </div>
</div>

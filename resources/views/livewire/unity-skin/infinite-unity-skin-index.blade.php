<div
    class="w-full grid grid-flow-dense grid-rows-[theme(spacing.12),4rem,70rem,1fr]
          ...
          min-[851px]:grid-rows-[theme(spacing.12),4rem,29rem,1fr]
    ...
    <div
        ...
        <div
            ...
            <a class="flex items-center justify-around cursor-pointer gap-x-2"
                href="https://www.youtube.com/watch?v=cuAsh_dMw90&ab_channel=BarbeDouce"
                title="Tutoriel pour poster un skin" target="_blank">
            ...
            <a href="{{ route('skinator.create') }}" target="_blank"
                class="w-fit mt-2 font-display text-secondary text-[1rem] bg-red-500 hover:bg-red-300 transition-all rounded-md px-4">{{ __('barbofus.contentAccessTo') }}
                skinator</a>
        </div>
    ...
    <x-skins.main-filter :races="$races" :winnersOnly="$winnersOnly" :barbOnly="$barbeOnly" :filterColor="$filterColor" :petTypeContent="$skinPetTypeWhere"
        :skinContent="$skinContentWhere" :gender="$genderWhere" :raceSelection="$raceWhere" :searchFilterInput="$searchFilterInput" :$raceWhere :canWinnersOnly="true" />

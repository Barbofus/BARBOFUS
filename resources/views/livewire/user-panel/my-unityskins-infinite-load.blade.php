<div x-data="{ skinDeleteID: null, skinDeleteImg: '', }">
        ...
        <div class="flex-1 flex flex-col items-center mb-10">
            @if(count($postIdChunks) > 0)
                @for($i = 0; $i < $page && $i < $maxPage; $i++)
                    <livewire:user-panel.my-unityskins-chunk :skinIds="$postIdChunks[$i]" :page="$page" :itemsPerPage="Self::ITEMS_PER_PAGE" :wire:key="'chunk-'.$queryCount.'-'.$i"/>
                @endfor
            @else
                <img class="mt-8 h-[16rem]" src="{{ asset('storage/images/misc_ui/Barbe_pleure.webp') }}">
                <p class="text-4xl font-normal">{{ __('barbofus.contentOuch') }} <span class="font-thin italic text-3xl">{{ __('barbofus.descriptionNoSkinPosted') }}</span></p>
            @endif
    ...
    <div class="fixed top-0 right-0 bottom-0 left-0"
        ...
        <x-utils.unity-skin-delete-verification />
    </div>
</div>

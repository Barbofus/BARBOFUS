<div x-data="{ havenBagDeleteID: null, havenBagDeleteImg: '', }">
    <div class="animate-topFade [--custom-translate-y:50px] [--custom-animation-time:0.3s]">
        ...
        <div class="sticky z-10 flex justify-center w-full py-8 bg-primary top-12">
            ...
            @foreach($havenBags as $key => $havenBag)
                <div class="group relative opacity-0 animate-skinApparition w-full h-full shadow-sm rounded-xl overflow-hidden {{ (($havenBag->status == 'Pending') ? 'bg-pendingBackground' : (($havenBag->status == 'Refused') ? 'bg-refusedBackground' : 'bg-primary-100')) }}">
                    ...
                    <div class="relative w-full overflow-hidden aspect-video">
                        ...
                        <div class="absolute flex justify-end w-full px-2 space-x-2 text-inactiveText top-2">
                            <a href="{{ route('havre-sacs.edit', $havenBag->id) }}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-10 transition-all hover:scale-125 group-hover:text-blue-500">
                            ...
                            <button x-on:click="havenBagDeleteID = {{ $havenBag->id }}, havenBagDeleteImg = '{{ asset('storage/' . $havenBag->image_path) }}'">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-10 transition-all hover:scale-125 group-hover:text-red-500">
